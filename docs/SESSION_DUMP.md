...
4. Terminology extractor pipeline: extract -> normalize/dedup -> split-to-body/suffix -> relation/review -> export.
5. Terminology output sheets: `terms_summary`, `relations_summary`, `review`, `details` (`terms_summary` now includes `term_type=body/suffix`).
6. Terminology extractor logs stage counters and timings:
- counters: `rows_scanned`, `rows_skipped_by_version`, candidate/normalized/body/suffix/relation/review counts
- timings: `config`, `discovery`, `extract`, `aggregate`, `export`, `total`
7. Update Master modes use one processor with policy combinations:
...
8. `Merge Masters` source duplicate behavior:
...
11. Update Master suite emits stage timing summaries:
...
- `save_master`
- `total`
12. Update Master suite column scope:
